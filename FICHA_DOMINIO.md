...
| **KPI de la EOD por ciudad** | Viajes diarios, viajes por persona, partición modal, propósito, tiempo mediano y distancia media, en total y sólo bicicleta | Ciudad-año EOD, 18 ciudades entre 2010 y 2023 | `eod_kpi` | **No admite lectura temporal**: cada ciudad tiene un año distinto. **Los campos heredados del índice `pct_bicicleta` y `pct_caminata` no se usan**: arrastran un defecto de homologación que clasificaba como caminata la categoría «No Caminata» del catálogo atípico de Gran Concepción y Curicó, sumándole la bicicleta. Gran Concepción quedó corregido aguas arriba el 2026-09-06; **Curicó no**, y sigue declarando 30,5 % de caminata y 0,0 % de bicicleta contra 21,8 % y 8,7 % del microdato. Usar `bici_pct_propio` y `cam_pct_propio`, que reproducen al índice en las 14 ciudades donde ya es consistente. Y su `pct_no_motorizado` en 0,0 % (Gran Valparaíso, San Antonio) es ausencia codificada como cero, no medición |
